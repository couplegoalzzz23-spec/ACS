...
# Tentukan direktori root projek secara dinamis agar aman di hosting cloud
...
# ==========================================
@st.cache_data
def load_acs_data(relative_filepath, categories):
    """
    Membaca 12 sheet Excel dengan jalur file yang dinamis agar tidak tersesat di cloud environment.
    """
    months = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni',
              'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
    data = []
    
    # Gabungkan BASE_DIR dengan relative path untuk menghasilkan absolute path yang absolut/pasti
    absolute_path = os.path.join(BASE_DIR, relative_filepath)
    
    if not os.path.exists(absolute_path):
        return pd.DataFrame(), f"File tidak ditemukan di path: {absolute_path}"

    try:
        for month in months:
            df = pd.read_excel(absolute_path, sheet_name=month, engine='openpyxl')
        ...
        return pd.DataFrame(), f"Error saat membaca {relative_filepath}: {str(e)}"
...
menu = st.sidebar.radio(
    ...
    key="acs_weather_parameters_radio"  # Explicit unique key to double-guard against duplicate IDs
)
